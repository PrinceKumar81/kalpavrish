...
void cmd_mkdir(const char* dirName) {
    if (findChild(cwd, dirName)) {
        printf("Error: '%s' already exists.\n", dirName);
        return;
    }
    insertChild(cwd, createNode(dirName, TYPE_DIRECTORY));
    printf("Directory '%s' created successfully.\n", dirName);
}

void cmd_create(const char* fileName) {
    if (findChild(cwd, fileName)) {
        printf("Error: '%s' already exists.\n", fileName);
        return;
    }
insertChild(cwd, createNode(fileName, TYPE_FILE));
...
void cmd_cd(const char* path) {
    ...
    if (!target || target->type != TYPE_DIRECTORY) {
...
int main() {
    initializeVFS();
    char input[256], command[50],  arg1[256], arg2[512];
    ...
    while (1){
        ...
        if (strcmp(command, "mkdir") == 0) cmd_mkdir(arg1);
        else if (strcmp(command, "create") == 0) cmd_create(arg1);
        else if (strcmp(command, "write") == 0) cmd_write(arg1, arg2);
        else if (strcmp(command, "read") == 0) cmd_read(arg1);
        else if (strcmp(command, "delete") == 0) cmd_delete(arg1);
        else if (strcmp(command, "rmdir") == 0) cmd_rmdir(arg1);
        else if (strcmp(command, "ls") == 0) cmd_ls();
        else if (strcmp(command, "cd") == 0) cmd_cd(arg1);
        else if (strcmp(command, "pwd") == 0) cmd_pwd();
        else if (strcmp(command, "df") == 0) cmd_df();
